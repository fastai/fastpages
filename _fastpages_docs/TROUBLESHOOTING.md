...
3. My initial build failed on GH-Pages Status 
    `Error messsage: Unable to build page. Please try again later.` `Error: Process completed with exit code 1.`
    If your github username contains capital letters e.g. MyUserName, go to [config file](_config.yml#L17) line 17 and rename `MyUserName.github.io` to `myusername.github.io`. After the commit blog should build without error.
